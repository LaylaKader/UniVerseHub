import { CreditCardIcon, UserIcon } from "@heroicons/react/24/outline"; // Import PlusIcon for the button
import { SetStateAction, useState } from "react";

import { useGetAllCounsellingQuery } from "../../redux/features/Admin Management/GetAllCounselling";
import MyCompleteCounsellingStudent from "./MyCompleteCounsellingStudent";
...
export default function FullCounsellingLayout() {
  const {
  ...
  return (
    <>
      ...
      <div className="w-full  max-w-2xl px-4 mb-12 "></div>
        ...
        <div className="relative w-full px-4">
          ...
          {currentTab === "All Counselling" &&
            ...
            filteredCounsellingSessions.length > 0 && (
              <AllCounsellingStudent></AllCounsellingStudent>
            )}

          {currentTab === "My Counselling" && !isLoading && !error && (
            <MyBookedCounsellingStudent></MyBookedCounsellingStudent>
          )}

          {currentTab === "My Completed Counselling" &&
            !isLoading &&
            !error && (
              <MyCompleteCounsellingStudent></MyCompleteCounsellingStudent>
            )}
        </div>
      </div>
    </>
